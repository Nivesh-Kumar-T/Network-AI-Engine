...
from utils.metadata import increment_hit_counter
from utils.logger import log_traffic
...
def classify_log(log: dict) -> dict:
    ...
    if is_blacklisted(src_ip):
        increment_hit_counter("blacklist")
        log_traffic(log, "REJECT", "Blacklisted", confidence=1.0)
        return {"decision": "REJECT", "reason": "Blacklisted"}

    if is_whitelisted(src_ip):
        increment_hit_counter("whitelist")
        log_traffic(log, "ALLOW", "Whitelisted", confidence=1.0)
    ...
    conf_score = round(confidence if predicted else 1 - confidence, 4)
    reason = f"{'Flagged' if predicted else 'Allowed'} with confidence {conf_score}"

    log_traffic(log, decision, reason, confidence=conf_score, explanation=explanation)
